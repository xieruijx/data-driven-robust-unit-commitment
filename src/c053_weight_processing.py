...
number = pd.read_csv('./data/processed/weight/number.txt', header=None)
number = number[0]
...
for i in range(number):
    weight_result[i, :] = np.loadtxt('./data/processed/weight/index_' + str(index_u_l_predict) + '_weight_' + str(i) + '.txt')
    if os.path.isfile('./data/processed/weight/index_' + str(index_u_l_predict) + '_cost_' + str(i) + '.txt'):
        cost_result[i, :, :] = np.loadtxt('./data/processed/weight/index_' + str(index_u_l_predict) + '_cost_' + str(i) + '.txt')
        for j in range(2):
...
print(Y)
# Y[Y == np.inf] = np.max(Y[Y < np.inf]) * 1.02 ##############################################
...
np.savetxt('./data/processed/combination/d053_weight.txt', weight)

numbers = np.arange(0, X.shape[0], 1)
print(numbers[Y == np.min(Y)])
