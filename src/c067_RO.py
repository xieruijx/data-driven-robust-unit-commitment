...
np.savetxt('./data/processed/weight118/index_' + str(index_u_l_predict) + '_weight_RO.txt', weight)
validation_cost, test_cost, sxb1, sxc1, LBUB1, time1 = optimization.weight2cost_RO(parameter, weight, index_u_l_predict, name_case)
print(time1)
cost = np.concatenate((validation_cost.reshape((-1, 1)), test_cost.reshape((-1, 1))), axis=1)
np.savetxt('./data/processed/weight118/index_' + str(index_u_l_predict) + '_cost_RO.txt', cost)
np.savetxt('./data/processed/weight118/index_' + str(index_u_l_predict) + '_LBUB1_RO.txt', LBUB1)
print(cost[:3, :])
